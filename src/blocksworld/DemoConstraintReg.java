...
public class DemoConstraintReg {
    public static void main(String[] args) {
        
        int nbBlocks = 6; 
        int nbPiles = 2;   
        
        RegularConstraints bwConstraint = new RegularConstraints(nbBlocks, nbPiles);
    ...
    private static void executeSolver(Solver solver, String solverName, RegularConstraints bwConstraint, int nbBlocks) {
        System.out.println("---------------- Execution de " + solverName + " pour une configuration reguliere ---------------------");
    ...
    private static void visualizeSolution(Map<Variable, Object> solution, RegularConstraints bwConstraint, int nbBlocks, String solverName) {
        BWState<Integer> state = makeBWState(solution, nbBlocks);
        
        
        SwingUtilities.invokeLater(() -> {
            BWIntegerGUI gui = new BWIntegerGUI(nbBlocks);
            JFrame frame = new JFrame("Configuration Régulière - " + solverName);
